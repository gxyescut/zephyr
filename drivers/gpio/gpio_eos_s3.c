				    ...
				    uint32_t *value)
{
	...
	*value = 0;
	...
	for (uint8_t gpio_num = 0; gpio_num < MAX_GPIOS; gpio_num++) {
		HAL_GPIO_Read(gpio_num, &pin_value);
		*value |= (pin_value == 1) ? BIT(gpio_num) : 0;
	}
					   ...
					   uint32_t value)
{
	...
	for (uint8_t gpio_num = 0; gpio_num < MAX_GPIOS; gpio_num++) {
		if (target_value & 1) {
			HAL_GPIO_Write(gpio_num, 1);
		} else {
			HAL_GPIO_Write(gpio_num, 0);
		}
