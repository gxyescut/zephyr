			     ...
			     const struct device **dev)
{
	*dev = device_get_binding(argv[1]);
	if (!*dev) {
		shell_error(shell, "FPGA device %s not found", argv[1]);
		return -ENODEV;
	}
	return 0;
}
...
static int cmd_on(const struct shell *shell, size_t argc, char **argv)
{
	...
	err = parse_common_args(shell, argv, &dev);
	if (err < 0) {
		return err;
	}

	shell_print(shell, "%s: turning on", dev->name);
...
static int cmd_off(const struct shell *shell, size_t argc, char **argv)
{
	...
	shell_print(shell, "%s: turning off", dev->name);
...
static int cmd_reset(const struct shell *shell, size_t argc, char **argv)
{
	...
	shell_print(shell, "%s: resetting FPGA", dev->name);
...
static int cmd_load(const struct shell *shell, size_t argc, char **argv)
{
	...
	shell_print(shell, "%s: loading bitstream", dev->name);
...
static int cmd_get_status(const struct shell *shell, size_t argc, char **argv)
{
	...
	shell_print(shell, "%s status: %d", dev->name, fpga_get_status(dev));
...
static int cmd_get_info(const struct shell *shell, size_t argc, char **argv)
{
	...
	shell_print(shell, "%s", fpga_get_info(dev));
